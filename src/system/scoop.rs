...
/// The target a Scoop shim points at.
///
/// A shim is a pair: a generated `<name>.exe` wrapper nobody signs, and a
/// `<name>.shim` text file naming the real binary. Verifying the wrapper would
/// report Scoop's own plumbing as unsigned; the target is what runs.
pub(crate) fn shim_target(text: &str) -> Option<String> {
...
/// The app a shim target belongs to: `<root>\apps\<app>\current\<binary>`.
/// `None` when the target lies outside the Scoop tree, which is the finding —
/// a shim on `PATH` pointing somewhere Scoop does not manage.
pub(crate) fn app_of_target(root: &std::path::Path, target: &str) -> Option<String> {
    // Both sides are normalised: `Path::join` uses the *host* separator, so on
    // any machine that is not Windows the root would come back with `/` while
    // the target read off disk carries `\`. Comparing them raw only happens to
    // work on Windows, which is exactly the kind of thing that passes locally
    // and fails in a test suite.
    let norm = |p: &str| p.to_ascii_lowercase().replace('/', "\\");
    let apps = norm(&root.join("apps").to_string_lossy());
    let prefix = format!("{}\\", apps.trim_end_matches('\\'));
...
pub fn scoop_inventory(opts: Opts) -> Result<Inventory> {
    ...
    // Per-binary trust. Only the shim *targets* are verified — the wrappers in
    // `shims\` are generated by Scoop and signed by nobody, so checking them
    // would report Scoop's own plumbing.
    let mut targets: Vec<(String, String)> = Vec::new(); // (app, path)
    let mut notes = Vec::new();
    if let Ok(entries) = std::fs::read_dir(root.join("shims")) {
        for e in entries.filter_map(std::result::Result::ok) {
            if e.path().extension().is_none_or(|x| x != "shim") {
                continue;
            }
            let Ok(text) = std::fs::read_to_string(e.path()) else {
                continue;
            };
            let Some(target) = shim_target(&text) else {
                continue;
            };
            match app_of_target(&root, &target) {
                Some(app) => targets.push((app, target)),
                None => notes.push(format!(
                    "shim '{}' on PATH points outside Scoop's tree ({target}) [High] — a shim \
                     is resolved before anything later on PATH",
...
mod tests {
    ...
    /// Verbatim `putty.shim` from the reference machine.
    #[test]
    fn a_shim_names_the_binary_it_stands_for() {
        assert_eq!(
            shim_target("path = \"C:\\Users\\alice\\scoop\\apps\\putty\\current\\putty.exe\"\n"),
            Some("C:\\Users\\alice\\scoop\\apps\\putty\\current\\putty.exe".to_string())
        );
        // Shims can carry arguments alongside the path.
        assert_eq!(
            shim_target("path = \"C:\\x\\a.exe\"\nargs = --flag\n"),
            Some("C:\\x\\a.exe".to_string())
        );
        assert_eq!(shim_target("args = --flag\n"), None);
    }

    /// A shim on PATH pointing outside Scoop's tree is resolved before anything
    /// later on PATH — that is the hijack this catches.
    #[test]
    fn a_shim_pointing_outside_the_tree_has_no_owning_app() {
        let root = std::path::Path::new("C:\\Users\\alice\\scoop");
        assert_eq!(
            app_of_target(root, "C:\\Users\\alice\\scoop\\apps\\putty\\current\\putty.exe"),
            Some("putty".to_string())
        );
        assert_eq!(app_of_target(root, "C:\\Users\\alice\\AppData\\Local\\Temp\\evil.exe"), None);
        // Case and separator differences must not defeat the match.
        assert_eq!(
            app_of_target(root, "c:/users/alice/scoop/apps/jq/current/jq.exe"),
            Some("jq".to_string())
        );
    }
