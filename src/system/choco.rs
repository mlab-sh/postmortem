...
pub fn choco_inventory(opts: Opts) -> Result<Inventory> {
    ...
    //
    // One walk per package collects both the install scripts and, when
    // signatures are wanted, the binaries to verify (it was two walks).
    let root = choco_root();
    let mut signals: HashMap<String, Vec<SysSignal>> = HashMap::new();
    let mut owned: Vec<(String, String)> = Vec::new();
    for (name, _) in &pkgs {
        let (code, bins) = package_files(&root, name, if opts.signatures { 12 } else { 0 });
        owned.extend(bins.into_iter().map(|b| (name.clone(), b)));
    ...
    if opts.signatures && !owned.is_empty() {
        ...
        let owners = super::authenticode::owners_by_path(&owned);
        for (name, _) in &pkgs {
            let mine = super::authenticode::owned_by(&verified, &owners, name);
...
/// A Chocolatey package's install code and binaries, from one walk of
/// `<root>\lib\<pkg>\`: the scripts it runs (see below, depth 3) and up to
/// `cap` binaries (depth 4).
///
/// The binaries are the payload under `lib\<pkg>\`; the redirectors in `bin\`
/// are generated by Chocolatey and unsigned, so they are not the subject.
/// Capped per package because signature verification costs roughly 120 ms a
/// file and a single package can ship hundreds.
///
/// The scripts' names vary in case on disk (`chocolateyInstall.ps1` and
/// `chocolateyinstall.ps1` both occur), so they are matched case-insensitively
/// rather than by exact name. `chocolateyBeforeModify` and the uninstall script
/// count too: they run with the same privileges as the install.
pub(crate) fn package_files(root: &str, pkg: &str, cap: usize) -> (String, Vec<String>) {
    let dir = std::path::Path::new(root).join("lib").join(pkg);
    let mut code = String::new();
    let mut bins = Vec::new();
    for entry in walkdir::WalkDir::new(&dir)
        .max_depth(if cap > 0 { 4 } else { 3 })
        .into_iter()
        .filter_map(std::result::Result::ok)
    {
        ...
        if bins.len() < cap {
            ...
            if matches!(ext.as_str(), "exe" | "dll" | "msi" | "msix" | "sys") {
                bins.push(path.to_string_lossy().into_owned());
            }
        }
    }
    (code, bins)
