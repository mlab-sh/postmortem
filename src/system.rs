...
pub fn inventory(manager: &str, opts: Opts) -> Result<Inventory> {
    match manager {
        ...
        "dnf" => dnf_inventory(opts),
        "nix" => nix_inventory(opts),
        "apk" => apk_inventory(opts),
...
/// expected to differ.
fn verify_line_is_tamper(line: &str) -> bool {
...
fn find_setuid_files() -> std::collections::HashSet<String> {
    Command::new("find")
